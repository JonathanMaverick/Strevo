...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        if (!socketRef.current) {
            socketRef.current = new WebSocket(`ws://${process.env.VITE_BACKEND_HOST}:${process.env.VITE_BACKEND_PORT}/api/v1/global-sockets/ws/testID`);
            socketRef.current?.addEventListener('message', (e) => {
                const message: SocketMessage<any> = e.data;

                switch (message.type) {
                    case SocketMessageType.StartStream: handleShowStreamToast(message.data); break;
                }
            });
        }
